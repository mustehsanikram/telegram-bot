...
A Telegram bot that runs the subscription side of a personal styling business.
It tracks client payments, grants and revokes access to a private styling channel
based on whether a subscription is current, and sends renewal reminders before a
subscription lapses. Backend only: there is no web UI, and Telegram is the whole
client surface.
...
Python 3.12 managed by [uv](https://docs.astral.sh/uv/). All commands run from
the repo root; `uv run` resolves the project virtualenv, so no manual activation
is needed.

- Install and sync deps: `uv sync`
- Run the bot (long polling): `uv run stylebot`
- Test: `uv run pytest`
...
A single combined `Verify` command has not been set up yet. Run `/ci` or `$ci` to
define one from the checks above and add matching automatic GitHub checks.
